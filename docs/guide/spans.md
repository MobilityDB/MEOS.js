...
```ts
const s = IntSpan.fromBounds(1, 10);              // [1, 10)  (lowerInc=true, upperInc=false)
...
a.isContainedIn(c);    // false ([1,5) not in [3,8))
c.contains(a);         // false
```
...
const m = a.minus(b);          // [1, 5)  (null if empty)
...
f.round(1);        // [1.5, 10.5)  (1 decimal place)
