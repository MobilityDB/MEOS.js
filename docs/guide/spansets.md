...
ss.numSpans();    // 2
ss.lower();       // 1   — lower bound of the first span
ss.upper();       // 12  — upper bound of the last span
ss.lowerInc();    // true
ss.upperInc();    // false
...
ss.width();          // 8  — sum of individual span widths: (5-1) + (12-8)
...
a.isAdjacent(b);         // true  — {[1,5),[8,12)} is adjacent to {[5,8)}
a.overlaps(s);           // true  — [3,6) overlaps [1,5)
a.isContainedIn(s);      // false
a.contains(s);           // false — [3,6) is not fully inside a
```
